                                  ...
                                  const char *buf, size_t count)
{
       ...
       if (!strncmp(buf, "1", strlen("1"))) {
               ...
               dev_info(dev, "%s set actpower 1 \n", __func__);
               sysfs_notify(&xiaomi_fp->dev->kobj, NULL, "actpower");
       }
       else if (!strncmp(buf, "0", strlen("0"))) {
               xiaomi_fp->actpower = 0;
               dev_info(dev, "%s set actpower 0 \n", __func__);
       }
       else if (!strncmp(buf, "2", strlen("2"))) {
               xiaomi_fp->actpower = 2;
               dev_info(dev, "%s set actpower 2 \n", __func__);
               sysfs_notify(&xiaomi_fp->dev->kobj, NULL, "actpower");
       }
       else {
               dev_err(dev,"failed to set actpower\n");
               return -EINVAL;
       }
       return count;
}

static DEVICE_ATTR(actpower, S_IRUSR | S_IWUSR, get_actpower_event, set_actpower_event);
